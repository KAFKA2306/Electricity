...
def latest_eia860m_url(html: bytes) -> str:
    ...
    urls = [urljoin(EIA860M_INDEX, href) for href in parser.hrefs if re.search(r"/eia860m/xls/[a-z]+_generator20\d{2}\.xlsx$", href, re.I)]
    if not urls:
...
def read_bulk_series(zip_bytes: bytes, wanted: set[str]) -> dict[str, dict[str, Any]]:
    """Compatibility helper for exact series tests and ad-hoc callers."""
    return read_us48_bulk(zip_bytes)[0] | {sid: obj for sid, obj in read_us48_bulk(zip_bytes)[2].items() if sid in wanted}


def read_us48_bulk(zip_bytes: bytes) -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as archive:
        ...
        with archive.open(names[0]) as source:
            for line in source:
                ...
                if sid == INTERCHANGE_ID or (sid.startswith("EBA.US48-ALL.") and "interchange" in str(obj.get("name") or "").lower() and obj.get("f") == "H"):
                    interchange[sid] = obj
    missing = set(TOTAL_SERIES.values()) - totals.keys()
    if missing:
        raise ValueError(f"EBA bulk missing totals {sorted(missing)}")
    if len(fuels) < 5:
        raise ValueError(f"EBA bulk exposed only {len(fuels)} US48 fuel-generation series: {sorted(fuels)}")
    if not interchange:
...
def daily_actual(series: dict[str, Any], days: int = 120) -> list[dict[str, Any]]:
    ...
    rows = [{"period": day, "value": round(sum(values), 3), "hour_count": len(values)} for day, values in sorted(grouped.items()) if len(values) >= 20]
    if len(rows) < 90:
...
def weekly(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [{"week_start": start, "value": round(sum(values), 3), "day_count": len(values)} for start, values in sorted(groups.items())]
...
def worksheet_records(ws: Any) -> list[dict[str, Any]]:
    for row_number, row in enumerate(ws.iter_rows(values_only=True), start=1):
        ...
        if "generator id" in normalized and ("plant id" in normalized or "plant code" in normalized):
            return [{headers[i]: value for i, value in enumerate(record) if i < len(headers) and headers[i]} for record in ws.iter_rows(min_row=row_number + 1, values_only=True) if any(value not in (None, "") for value in record)]
        if row_number >= 15:
            break
...
def capacity_summary(xlsx: bytes) -> dict[str, Any]:
    ...
    for sheet_name in SHEETS:
        ...
        for record in records:
            s, n = number(record.get(summer)), number(record.get(nameplate))
            if s is not None:
                summer_total += s
                by_technology[str(record.get(technology) or "Unknown")] += s
            if n is not None:
                nameplate_total += n
        result[sheet_name.lower()] = {"generator_count": len(records), "nameplate_capacity_mw": round(nameplate_total, 3), "net_summer_capacity_mw": round(summer_total, 3), "by_technology_net_summer_mw": dict(sorted(by_technology.items()))}
    return result
...
def build(days: int) -> dict[str, Any]:
    ...
    views = {name: series_view(name, totals[sid], days) for name, sid in TOTAL_SERIES.items()}
    ...
    if len(fuel_views) < 5:
        raise ValueError(f"fewer than five 90-day fuel series survived: {sorted(fuel_views)}; excluded={excluded}")
    common = set.intersection(*(set(row["period"] for row in item["daily"]) for item in fuel_views.values()))
    if not common:
        raise ValueError("fuel-generation series have no common daily period")
    latest = max(common)
    latest_values = {code: next(row["value"] for row in item["daily"] if row["period"] == latest) for code, item in fuel_views.items()}
    ...
    generation_mix = {
        ...
        "latest": {"period": latest, "total_mwh": round(total, 3), "by_fuel_mwh": latest_values, "share": {code: round(value / total, 8) for code, value in latest_values.items()} if total else {}},
        "source_url": EBA_URL,
    }
    ...
    return {
        ...
        "sources": {"eba_bulk": {"url": EBA_URL, "sha256": sha(eba)}, "eia860m": {"index_url": EIA860M_INDEX, "url": capacity_url, "sha256": sha(xlsx), "sheets": list(SHEETS)}},
        "views": views,
...
def publish(evidence_dir: Path, api_dir: Path, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for name, view in stored["views"].items():
        published[name] = {**view, "source_evidence": str(evidence.relative_to(ROOT)) if evidence.is_relative_to(ROOT) else str(evidence), "source_evidence_sha256": evidence_sha}
    published["generation-mix"] = {**stored["generation_mix"], "source_evidence": str(evidence.relative_to(ROOT)) if evidence.is_relative_to(ROOT) else str(evidence), "source_evidence_sha256": evidence_sha}
    published["capacity"] = {"schema_version": 1, "dataset": "capacity", "frequency": "monthly", "kind": "reported-inventory", "unit": "MW", "geography": {"level": "generator", "coverage": "United States and Puerto Rico where reported"}, **stored["capacity"], "source_url": stored["sources"]["eia860m"]["url"], "source_evidence": str(evidence.relative_to(ROOT)) if evidence.is_relative_to(ROOT) else str(evidence), "source_evidence_sha256": evidence_sha}
    published["capacity-additions"] = {"schema_version": 1, "dataset": "capacity-additions", "frequency": "monthly", "kind": "reported-inventory", "unit": "MW", "planned": stored["capacity"]["planned"], "retired": stored["capacity"]["retired"], "source_url": stored["sources"]["eia860m"]["url"], "source_evidence": str(evidence.relative_to(ROOT)) if evidence.is_relative_to(ROOT) else str(evidence), "source_evidence_sha256": evidence_sha}

    for name, view in published.items():
        (api_dir / f"{name}.json").write_bytes(dump(view))
    (api_dir / "index.json").write_bytes(dump({"schema_version": 1, "publisher": stored["publisher"], "namespace": "electricity", "datasets": {name: {"path": f"{name}.json", "frequency": view["frequency"], "kind": view["kind"], "source_url": view["source_url"]} for name, view in sorted(published.items())}}))
    (api_dir.parent / "energy-supply.json").write_bytes(dump({"schema_version": 1, "theme": "ARK Big Ideas 2026 / Distributed Energy", "domains": {"petroleum": {"namespace": "petroleum", "path": "latest.json"}, "electricity": {"namespace": "electricity", "path": "electricity/index.json"}}, "rule": "Domains are linked by reference only; units and values are never merged."}))
    return published
...
def main() -> None:
    ...
    print(json.dumps({"demand_days": views["demand"]["period_count"], "generation_days": views["generation"]["period_count"], "interchange_days": views["interchange"]["period_count"], "fuel_count": views["generation-mix"]["fuel_count"], "operating_generators": views["capacity"]["operating"]["generator_count"], "planned_generators": views["capacity"]["planned"]["generator_count"], "retired_generators": views["capacity"]["retired"]["generator_count"]}, sort_keys=True))
